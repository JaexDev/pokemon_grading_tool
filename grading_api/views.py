...
class PokemonCardFilter(filters.FilterSet):
    ...
    class Meta:
        model = PokemonCard
        fields = ['card_name', 'set_name', 'tcgplayer_price_min', 'tcgplayer_price_max', 'psa_10_price_min', 'psa_10_price_max']
...
class PokemonCardViewSet(viewsets.ModelViewSet):
    ...
    def scrape_and_save(self, request):
        ...
        try:
            ...
            tcgplayer_data = scraper.get_tcgplayer_data(search_query, search_query, language)
            ...
            all_cards = []
            for card_data in tcgplayer_data:
                logger.info(f"Fetching eBay price for {card_data.get('card_name')} in {card_data.get('set_name')}")
                ebay_price = scraper.get_ebay_psa10_price(card_data.get("card_name"), card_data.get("set_name"))
                if ebay_price:
                    logger.info(f"Calculating profits for {card_data.get('card_name')} in {card_data.get('set_name')}")
                    profit_data = scraper.calculate_profit([card_data], ebay_price)
                    if profit_data:
                        for card in profit_data:
                            logger.info(f"Creating or updating record for {card.get('card_name')} in {card.get('set_name')}, {card.get('rarity')}, with tcgplayer_price: {card.get('tcgplayer_price')}, psa_10_price: {card.get('psa_10_price')}, price_delta: {card.get('price_delta')}, profit_potential: {card.get('profit_potential')}")
                        ...
                        logger.error(f"Could not calculate profits for {card_data.get('card_name')} in {card_data.get('set_name')}")
                        return Response({'error': 'Could not calculate profits'}, status=404)
                else:
                    logger.error(f"Could not get ebay data for {card_data.get('card_name')} in {card_data.get('set_name')}")
                    return Response({'error': 'Could not get ebay data'}, status=404)
